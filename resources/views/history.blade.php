                ...
                @foreach($notifications as $notif)
                    <li class="dropdown-header" style="padding: 0.5rem 0.5rem !important; word-break: break-word; width:100%; overflow:hidden; text-overflow:ellipsis;">
                        @if($notif['type'] === 'App\Notifications\NewFollower')
                            <a href="/profile/{{$notif['data']['following_id']}}"><i class="fa fa-users" aria-hidden="true"></i>&nbsp; {{\Carbon\Carbon::parse($notif['created_at'])->diffforHumans()}} : <b class="text-primary">{{$notif['data']['follower_name']}}</b> follow you!</a>
