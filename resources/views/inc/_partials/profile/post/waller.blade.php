<div class="card gedf-card-post">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="ml-2">
                    <div class="h5 m-0"><a href="{{route('profile', [$poster->waller_id])}}" class="card-link text-muted pointer" title="profile">{{$poster->waller->name}}</a> <i class="fa fa-angle-double-right" aria-hidden="true"></i> <a href="{{route('profile', [$poster->user_id])}}" class="card-link text-muted" title="profile">{{$poster->name}}</a></div>
                    @if(\auth::check())
                        ...
                        @endif
                    @else
                        <div class="h7">@<a href="#" class="card-link">{{$poster->waller->username}}</a> <i class="fa fa-angle-double-right" aria-hidden="true"></i> @<a href="#" class="card-link">{{$poster->username}}</a></div>
                    @endif
                </div>
            </div>
            <div>
    ...
    <div class="card-footer">        
        <span id="like{{$poster->id}}-bs3" class="thumb">{{ App\Post::find($poster->id)->likers()->get()->count() }}</span>
        @if(\auth::check())
